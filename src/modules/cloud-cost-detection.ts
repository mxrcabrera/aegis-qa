...
export class CloudCostDetection {
  ...
  private highTrafficFiles: string[] = [];
   ...
   */
  async detect(
    apiPayloadData: Map<string, string> = new Map(),
   ...
   */
  private async detectZombieDependencies(): Promise<CostIssue[]> {
   ...
   */
  private applyCostImpactMultiplier(issues: CostIssue[]): void {
    for (const issue of issues) {
      ...
      if (isCriticalModule && hasHighTraffic) {
        // Triple the severity
        if (issue.severity === 'low') issue.severity = 'high';
        else if (issue.severity === 'medium') issue.severity = 'critical';
   ...
   */
  private async analyzeFile(filePath: string, apiPayloadData: Map<string, string>): Promise<CostIssue[]> {
    const issues: CostIssue[] = [];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (heavyColdStart) issues.push(heavyColdStart);
      ...
      if (selectStar) issues.push(selectStar);
      ...
      if (isCorePath) {
        ...
        if (logProfligacy) issues.push(logProfligacy);
      }
      ...
      if (inefficientRegex) issues.push(inefficientRegex);
    ...
    if (sequentialAwaits.length > 2) {
      ...
      if (longRunning) issues.push(longRunning);
    }
    ...
    if (apiCallsInLoops.length > 0) {
      ...
      if (apiLoop) issues.push(apiLoop);
    }
   ...
   */
  private detectHeavyColdStart(line: string, filePath: string, lineNumber: number): CostIssue | null {
   ...
   */
  private detectLongRunningFunction(filePath: string, awaitLines: number[]): CostIssue | null {
    if (awaitLines.length < 3) return null;
   ...
   */
  private detectSelectStarAbuse(line: string, filePath: string, lineNumber: number): CostIssue | null {
    // SQL select *
   ...
   */
  private detectPayloadBloat(line: string, filePath: string, lineNumber: number, apiPayloadData: Map<string, string>): CostIssue | null {
    // Detect API calls
    const apiCallMatch = line.match(/fetch\(|axios\.|\.get\(|\.post\(/);
   ...
   */
  private detectLogProfligacy(line: string, filePath: string, lineNumber: number): CostIssue | null {
   ...
   */
  private detectInefficientRegex(line: string, filePath: string, lineNumber: number): CostIssue | null {
   ...
   */
  private detectApiLoopInefficiency(filePath: string, loopLines: number[]): CostIssue | null {
    if (loopLines.length < 2) return null;
   ...
   */
  private detectMissingCacheControl(line: string, filePath: string, lineNumber: number): CostIssue | null {
   ...
   */
  private calculateSummary(issues: CostIssue[]): CostResults['summary'] {
    return {
