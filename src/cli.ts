...
class AegisCLI {
  ...
  async run(): Promise<void> {
    ...
    if (!this.config.ciMode) {
      console.log('≠É¯Ì¥©≈  Aegis QA - Advanced Quality Assurance Orchestrator');
      console.log(`≠ÉÙÈ Target Directory: ${resolve(targetDir)}\n`);
    }
    ...
    if (!this.config.ciMode) {
      console.log('≠Éˆº Establishing error baseline...');
    }
    ...
    if (!this.config.skipThermal) {
      if (!this.config.ciMode) {
        console.log('≠Éˆº Running self-diagnostic stress test...');
      }
    ...
    if (!this.config.ciMode) {
      console.log('≠Éˆ∫ Detecting hardware capabilities...');
    }
  ...
  private async runReview(phaseOrchestrator: PhaseOrchestrator, reportAggregator: ReportAggregator): Promise<void> {
    ...
    if (result.success) {
      ...
      if (!this.config.ciMode) {
        console.log('\n‘£‡ Review Complete');
        console.log(`≠ÉÙË Total Findings: ${result.totalFindings}`);
        console.log(`‘≈¶¥©≈  Total Time: ${(result.totalExecutionTimeMs / 1000).toFixed(2)}s`);
      ...
      if (this.config.sandboxMode) {
        ...
        console.log(`≠ÉÂ New Issues: ${newViolationCount}`);
        console.log(`≠É• Inherited Issues: ${inheritedViolationCount}`);
      } else {
      ...
      if (newViolationCount > 0) {
        console.log('\n‘ÿÓ New issues detected');
        process.exit(1);
      } else if (inheritedViolationCount > 0) {
        console.log('\n‘£‡ No new issues (all errors inherited)');
        process.exit(0);
      }
    } else {
      console.log('\n‘ÿÓ Review Failed');
  ...
  private async runFix(phaseOrchestrator: PhaseOrchestrator, reportAggregator: ReportAggregator): Promise<void> {
    if (!this.config.ciMode) {
      console.log('≠Éˆ∫ Running Atomic Fixes (Phases 16-18)\n');
    }
    ...
    if (result.success) {
      ...
      if (!this.config.ciMode) {
        ...
        console.log(`‘ÿÓ Failed: ${result.failedCount}`);
        console.log(`≠ÉÂ New Issues: ${newViolationCount}`);
        console.log(`≠É• Inherited Issues: ${inheritedViolationCount}`);
  ...
  private async runIncremental(phaseOrchestrator: PhaseOrchestrator, reportAggregator: ReportAggregator): Promise<void> {
    if (!this.config.ciMode) {
      console.log('≠Éˆ‰ Running Incremental Review (Phase 19)\n');
    }
