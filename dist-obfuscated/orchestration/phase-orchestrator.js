...
import { Phase16FixStrategyGeneration } from '../phases/phase-16-fix-strategy-generation.js';
import { Phase17MultiFixExecution } from '../phases/phase-17-multi-fix-execution.js';
import { Phase18PostFixValidation } from '../phases/phase-18-post-fix-validation.js';
import { Phase19IncrementalReview } from '../phases/phase-19-incremental-review.js';
...
export class PhaseOrchestrator {
    ...
    sandboxManager = null;
    gcAvailable;
    memoryThreshold = 1.5 * 1024 * 1024 * 1024; // 1.5GB
    dryRunMode;
    yesMode;
    activeTimeouts = new Set();
    globalExecutionStartTime = 0;
    globalRuntimeLimitMs;
    constructor(config) {
        this.config = config;
     ...
     */
    handleGlobalTimeoutAbort(phaseResults, totalPhases) {
        const elapsed = Date.now() - this.globalExecutionStartTime;
     ...
     */
    generatePartialReport(phaseResults, elapsed, totalFindings) {
     ...
     */
    validateDryRunForOperation(operation) {
        if (this.dryRunMode) {
     ...
     */
    async triggerGarbageCollection(phaseName) {
        ...
        try {
            ...
            if (heapUsed > this.memoryThreshold) {
                ...
                console.warn(`?? Memory Guard: Heap usage ${heapUsedMB.toFixed(2)} MB exceeds threshold ${thresholdMB} MB`);
                console.warn(`?? Pausing for 5 seconds to let OS breathe...`);
                // Pause for 5 seconds
                ...
                if (afterPause.heapUsed > this.memoryThreshold) {
                    console.warn(`?? Memory still above threshold after pause. Monitor closely.`);
                }
                else {
    ...
    async checkThermalLock() {
        try {
            ...
            if (isCritical) {
                ...
                console.warn('?? Thermal conditions did not improve after 5 minutes - Forcing lock release');
                console.warn('?? Continuing operations at risk - Monitor hardware closely');
     ...
     */
    async runFullReview() {
        console.log('������  Aegis QA - Full Review Mode');
        ...
        try {
            ...
            const phase0Result = await this.runWithTimeout(() => phase0Setup.execute(), timeoutMs, 'Phase 0: Setup');
            if (!phase0Result.success) {
                console.error(`��� Phase 0 failed: ${phase0Result.error}`);
                console.error('Setup failed - project cannot proceed. Fix the issues above and try again.');
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(0, 'Setup', 0, Date.now() - phase0StartTime, phase0Result.error);
                }
                return {
                    success: false,
            ...
            console.log('ԣ� Phase 0: Setup passed\n');
            if (this.config.enablePartialReports) {
                await this.writePartialReport(0, 'Setup', 0, phase0Result.executionTimeMs);
            }
            ...
            console.error(`��� Phase 0 failed with exception: ${errorMessage}`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(0, 'Setup', 0, Date.now() - phase0StartTime, errorMessage);
            }
            return {
                success: false,
                totalExecutionTimeMs: Date.now() - startTime,
        ...
        console.log('���� Phase 1: Code Quality - Technical Health Assessment');
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase1Result = await this.runWithTimeout(() => phase1CodeQuality.execute(), timeoutMs, 'Phase 1: Code Quality');
            if (!phase1Result.success) {
                console.error(`��� Phase 1 failed: ${phase1Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(1, 'Code Quality', 0, Date.now() - phase1StartTime, phase1Result.error);
                }
                phaseResults.push({
                    phase: 1,
                ...
                console.log(`ԣ� Phase 1: Code Quality passed`);
                console.log(`  ���� Files analyzed: ${phase1Result.totalFiles}`);
                console.log(`  ���� Total findings: ${phase1Result.totalFindings}`);
                console.log(`  ��ᴩ�  Critical files: ${phase1Result.criticalFiles.length}`);
                console.log(`  ���� Average quality score: ${phase1Result.averageScore.toFixed(1)}/100\n`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(1, 'Code Quality', phase1Result.totalFindings, phase1Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 1 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(1, 'Code Quality', 0, Date.now() - phase1StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 1,
                phaseName: 'Code Quality',
        ...
        console.log('��Ļ Phase 2: Business Logic - Business Semantics & Core Path Detection');
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase2Result = await this.runWithTimeout(() => phase2BusinessLogic.execute(), timeoutMs, 'Phase 2: Business Logic');
            if (!phase2Result.success) {
                console.error(`��� Phase 2 failed: ${phase2Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(2, 'Business Logic', 0, Date.now() - phase2StartTime, phase2Result.error);
                }
                phaseResults.push({
                    phase: 2,
                ...
                console.log(`ԣ� Phase 2: Business Logic passed`);
                console.log(`  ��Ļ Domain: ${phase2Result.businessProfile.domain}`);
                console.log(`  ���� Confidence: ${phase2Result.businessProfile.confidence}%`);
                console.log(`  ���� Critical modules: ${phase2Result.businessProfile.criticalModules.length}`);
                console.log(`  ��ᴩ�  Risk findings: ${phase2Result.businessProfile.riskFindings.length}\n`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(2, 'Business Logic', phase2Result.businessProfile.riskFindings.length, phase2Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 2 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(2, 'Business Logic', 0, Date.now() - phase2StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 2,
                phaseName: 'Business Logic',
        ...
        // Phase 3: Security
        console.log('���� Phase 3: Security - Vulnerability & Secret Detection');
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase3Result = await this.runWithTimeout(() => phase3Security.execute(), timeoutMs, 'Phase 3: Security');
            if (!phase3Result.success) {
                console.error(`��� Phase 3 failed: ${phase3Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(3, 'Security', 0, Date.now() - phase3StartTime, phase3Result.error);
                }
                phaseResults.push({
                    phase: 3,
                    phaseName: 'Security',
                ...
                console.log(`ԣ� Phase 3: Security passed`);
                console.log(`  ���� Total findings: ${phase3Result.findings.length}`);
                console.log(`  ��ܿ Critical findings: ${phase3Result.criticalFindings}`);
                console.log(`  ��ᴩ�  High severity findings: ${phase3Result.highSeverityFindings}\n`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(3, 'Security', phase3Result.findings.length, phase3Result.executionTimeMs);
                }
                ...
                if (this.config.currentState.highRiskBlocker) {
                    console.error(`��ܿ��ܿ��ܿ HIGH RISK BLOCKER ACTIVE ��ܿ��ܿ��ܿ`);
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 3 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(3, 'Security', 0, Date.now() - phase3StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 3,
                phaseName: 'Security',
        ...
        // Phase 4: Database
        console.log('���䴩�  Phase 4: Database - Schema & Query Audit');
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase4Result = await this.runWithTimeout(() => phase4Database.execute(), timeoutMs, 'Phase 4: Database');
            if (!phase4Result.success) {
                console.error(`��� Phase 4 failed: ${phase4Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(4, 'Database', 0, Date.now() - phase4StartTime, phase4Result.error);
                }
                phaseResults.push({
                    phase: 4,
                    phaseName: 'Database',
                ...
                console.log(`ԣ� Phase 4: Database passed`);
                console.log(`  ���� Total findings: ${phase4Result.findings.length}`);
                console.log(`  ��ܿ Critical findings: ${phase4Result.criticalFindings}`);
                console.log(`  ��ᴩ�  High severity findings: ${phase4Result.highSeverityFindings}`);
                console.log(`  ��� Detected ORM: ${phase4Result.ormType}\n`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(4, 'Database', phase4Result.findings.length, phase4Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 4 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(4, 'Database', 0, Date.now() - phase4StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 4,
                phaseName: 'Database',
        ...
        // Phase 5: Clean Code
        console.log('ԣ� Phase 5: Clean Code & Refactoring');
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase5Result = await this.runWithTimeout(() => phase5CleanCode.execute(), timeoutMs, 'Phase 5: Clean Code');
            if (!phase5Result.success) {
                console.error(`��� Phase 5 failed: ${phase5Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(5, 'Clean Code', 0, Date.now() - phase5StartTime, phase5Result.error);
                }
                phaseResults.push({
                    phase: 5,
                ...
                console.log(`ԣ� Phase 5: Clean Code passed`);
                console.log(`  ���� Total findings: ${phase5Result.findings.length}`);
                console.log(`  ��ܿ High severity findings: ${phase5Result.highSeverityFindings}`);
                console.log(`  ��ᴩ�  Medium severity findings: ${phase5Result.mediumSeverityFindings}`);
                console.log(`  ���� Files analyzed: ${phase5Result.filesAnalyzed}\n`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(5, 'Clean Code', phase5Result.findings.length, phase5Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 5 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(5, 'Clean Code', 0, Date.now() - phase5StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 5,
                phaseName: 'Clean Code',
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase6Result = await this.runWithTimeout(() => phase6APIContracts.execute(), timeoutMs, 'Phase 6: API & Contracts');
            if (!phase6Result.success) {
                console.error(`��� Phase 6 failed: ${phase6Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(6, 'API & Contracts', 0, Date.now() - phase6StartTime, phase6Result.error);
                }
                phaseResults.push({
                    phase: 6,
                ...
                console.log(`ԣ� Phase 6: API & Contracts passed`);
                console.log(`  ���� Total findings: ${phase6Result.findings.length}`);
                console.log(`  ��ܿ Critical findings: ${phase6Result.criticalFindings}`);
                console.log(`  ��ᴩ�  High severity findings: ${phase6Result.highSeverityFindings}`);
                console.log(`  ���� Files analyzed: ${phase6Result.filesAnalyzed}\n`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(6, 'API & Contracts', phase6Result.findings.length, phase6Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 6 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(6, 'API & Contracts', 0, Date.now() - phase6StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 6,
                phaseName: 'API & Contracts',
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase7Result = await this.runWithTimeout(() => phase7TestingStrategy.execute(), timeoutMs, 'Phase 7: Testing Strategy');
            if (!phase7Result.success) {
                console.error(`��� Phase 7 failed: ${phase7Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(7, 'Testing Strategy', 0, Date.now() - phase7StartTime, phase7Result.error);
                }
                phaseResults.push({
                    phase: 7,
                ...
                console.log(`ԣ� Phase 7: Testing Strategy passed`);
                console.log(`  ���� Total findings: ${phase7Result.findings.length}`);
                console.log(`  ��ܿ Critical findings: ${phase7Result.criticalFindings}`);
                console.log(`  ��ᴩ�  High severity findings: ${phase7Result.highSeverityFindings}`);
                console.log(`  ���� Source files: ${phase7Result.filesAnalyzed}, Test files: ${phase7Result.testFilesFound}\n`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(7, 'Testing Strategy', phase7Result.findings.length, phase7Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 7 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(7, 'Testing Strategy', 0, Date.now() - phase7StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 7,
                phaseName: 'Testing Strategy',
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase8Result = await this.runWithTimeout(() => phase8Performance.execute(), timeoutMs, 'Phase 8: Performance & Scalability');
            if (!phase8Result.success) {
                console.error(`��� Phase 8 failed: ${phase8Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(8, 'Performance & Scalability', 0, Date.now() - phase8StartTime, phase8Result.error);
                }
                phaseResults.push({
                    phase: 8,
                ...
                console.log(`ԣ� Phase 8: Performance & Scalability passed`);
                console.log(`  ���� Total findings: ${phase8Result.findings.length}`);
                console.log(`  ��ܿ Critical findings: ${phase8Result.criticalFindings}`);
                console.log(`  ��ᴩ�  High severity findings: ${phase8Result.highSeverityFindings}\n`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(8, 'Performance & Scalability', phase8Result.findings.length, phase8Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 8 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(8, 'Performance & Scalability', 0, Date.now() - phase8StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 8,
                phaseName: 'Performance & Scalability',
        ...
        console.log('���� Phase 9: Internationalization & Accessibility (i18n & a11y)');
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase9Result = await this.runWithTimeout(() => phase9I18nA11y.execute(), timeoutMs, 'Phase 9: Internationalization & Accessibility');
            if (!phase9Result.success) {
                console.error(`��� Phase 9 failed: ${phase9Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(9, 'Internationalization & Accessibility', 0, Date.now() - phase9StartTime, phase9Result.error);
                }
                phaseResults.push({
                    phase: 9,
                ...
                console.log(`ԣ� Phase 9: Internationalization & Accessibility passed`);
                console.log(`  ���� Total findings: ${phase9Result.findings.length}`);
                console.log(`  ��ܿ Critical findings: ${phase9Result.criticalFindings}`);
                console.log(`  ��ᴩ�  High severity findings: ${phase9Result.highSeverityFindings}\n`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(9, 'Internationalization & Accessibility', phase9Result.findings.length, phase9Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 9 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(9, 'Internationalization & Accessibility', 0, Date.now() - phase9StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 9,
                phaseName: 'Internationalization & Accessibility',
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase10Result = await this.runWithTimeout(() => phase10EnvCICD.execute(), timeoutMs, 'Phase 10: Environment & CI/CD');
            if (!phase10Result.success) {
                console.error(`��� Phase 10 failed: ${phase10Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(10, 'Environment & CI/CD', 0, Date.now() - phase10StartTime, phase10Result.error);
                }
                phaseResults.push({
                    phase: 10,
                ...
                console.log(`ԣ� Phase 10: Environment & CI/CD passed`);
                console.log(`  ���� Total findings: ${phase10Result.findings.length}`);
                console.log(`  ��ܿ Critical findings: ${phase10Result.criticalFindings}`);
                console.log(`  ��ᴩ�  High severity findings: ${phase10Result.highSeverityFindings}\n`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(10, 'Environment & CI/CD', phase10Result.findings.length, phase10Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 10 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(10, 'Environment & CI/CD', 0, Date.now() - phase10StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 10,
                phaseName: 'Environment & CI/CD',
        ...
        console.log('���� Phase 11: Testing Deep Audit');
        ...
        try {
            ...
            console.log('��ᴩ�  Pre-flight Check: Establishing error baseline...');
            await this.errorBaseline.establishBaseline();
            // Git Checkpointing: Create checkpoint before applying fixes
            console.log('��ᴩ�  Git Checkpointing: Creating safety checkpoint...');
            ...
            if (isInGitRepo) {
                try {
                    ...
                    console.log(`ԣ� Checkpoint created: ${checkpoint.tagName} (${checkpoint.commitHash})`);
                }
                catch (error) {
                    console.warn('��ᴩ�  Failed to create git checkpoint, proceeding without rollback capability');
                    console.warn('��ᴩ�  Error:', error instanceof Error ? error.message : String(error));
                }
            }
            else {
                console.warn('��ᴩ�  Not in a git repository, skipping checkpoint');
            }
            ...
            if (!isDryRunMode) {
                console.log('��ᴩ�  Safe Level 4: Checking for high-risk files...');
                // TODO: Implement dependency graph analysis here
                // For now, force dry-run mode for safety
                console.warn('��ᴩ�  Auto-fix mode not yet implemented. Running in dry-run mode for safety.');
            }
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase11Result = await this.runWithTimeout(() => phase11AtomicFixes.execute(), timeoutMs, 'Phase 11: Atomic Fixes');
            if (!phase11Result.success) {
                console.error(`��� Phase 11 failed: ${phase11Result.error}`);
                console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(11, 'Testing Deep Audit', 0, Date.now() - phase11StartTime, phase11Result.error);
                }
                phaseResults.push({
                    phase: 11,
                ...
                console.log(`ԣ� Phase 11: Atomic Fixes passed`);
                console.log(`  ���� Total fixes attempted: ${phase11Result.remediationResult.totalFixesAttempted}`);
                console.log(`  ��ܿ Fixes applied: ${phase11Result.remediationResult.fixesApplied}`);
                console.log(`  ��ᴩ�  Fixes skipped: ${phase11Result.remediationResult.fixesSkipped}\n`);
                // Post-fix validation: Run tsc --noEmit to check if fixes broke the build
                console.log('��ᴩ�  Post-fix validation: Running tsc --noEmit...');
                ...
                try {
                    ...
                    if (stderr && stderr.trim().length > 0) {
                        console.error('��� Post-fix validation failed: tsc --noEmit found errors');
                        console.error('��� Rolling back to checkpoint...');
                        // Rollback to checkpoint
                        try {
                            await this.gitCheckpointManager.rollbackToLastCheckpoint();
                            console.log('ԣ� Successfully rolled back to checkpoint');
                            ...
                            if (this.config.enablePartialReports) {
                                await this.writePartialReport(11, 'Atomic Fixes', 0, phase11Result.executionTimeMs, 'Post-fix validation failed: tsc --noEmit found errors. Rolled back to checkpoint.');
                            }
                            ...
                            console.error('��� Failed to rollback to checkpoint:', rollbackError);
                            console.error('��� Manual intervention may be required to restore repository state');
                        }
                    }
                    else {
                        console.log('ԣ� Post-fix validation passed');
                    }
                }
                catch {
                    console.warn('��ᴩ�  Could not run tsc --noEmit for validation, skipping post-fix check');
                }
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(11, 'Atomic Fixes', phase11Result.remediationResult.fixesApplied, phase11Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 11 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(11, 'Testing Deep Audit', 0, Date.now() - phase11StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 11,
                phaseName: 'Testing Deep Audit',
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase12Result = await this.runWithTimeout(() => phase12ErrorHandling.execute(), timeoutMs, 'Phase 12: Error Handling & Observability');
            if (!phase12Result.success) {
                ...
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(12, 'Error Handling & Observability', 0, Date.now() - phase12StartTime, phase12Result.error);
                }
                phaseResults.push({
                    phase: 12,
                ...
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(12, 'Error Handling & Observability', phase12Result.errorFindings.length, phase12Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 12 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(12, 'Error Handling & Observability', 0, Date.now() - phase12StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 12,
                phaseName: 'Error Handling & Observability',
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase13Result = await this.runWithTimeout(() => phase13I18nL10n.execute(), timeoutMs, 'Phase 13: i18n & l10n');
            if (!phase13Result.success) {
                console.error(`ERROR Phase 13 failed: ${phase13Result.error}`);
                console.log(`WARNING Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(13, 'i18n & l10n', 0, Date.now() - phase13StartTime, phase13Result.error);
                }
                phaseResults.push({
                    phase: 13,
                    phaseName: 'i18n & l10n',
                ...
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(13, 'i18n & l10n', phase13Result.i18nFindings.length, phase13Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 13 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(13, 'i18n & l10n', 0, Date.now() - phase13StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 13,
                phaseName: 'i18n & l10n',
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase14Result = await this.runWithTimeout(() => phase14GitHygiene.execute(), timeoutMs, 'Phase 14: Git, Repo & Documentation Hygiene');
            if (!phase14Result.success) {
                console.error(`ERROR Phase 14 failed: ${phase14Result.error}`);
                console.log(`WARNING Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(14, 'Git, Repo & Documentation Hygiene', 0, Date.now() - phase14StartTime, phase14Result.error);
                }
                phaseResults.push({
                    phase: 14,
                ...
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(14, 'Git, Repo & Documentation Hygiene', phase14Result.gitHygieneFindings.length, phase14Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 14 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(14, 'Git, Repo & Documentation Hygiene', 0, Date.now() - phase14StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 14,
                phaseName: 'Git, Repo & Documentation Hygiene',
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase15Result = await this.runWithTimeout(() => phase15CICDDevOps.execute(), timeoutMs, 'Phase 15: CI/CD & DevOps');
            if (!phase15Result.success) {
                ...
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(15, 'CI/CD & DevOps', 0, Date.now() - phase15StartTime, phase15Result.error);
                }
                phaseResults.push({
                    phase: 15,
                ...
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(15, 'CI/CD & DevOps', phase15Result.cicdFindings.length, phase15Result.executionTimeMs);
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`��� Phase 15 failed with exception: ${errorMessage}`);
            console.error(`��ᴩ�  Continuing to final summary (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(15, 'CI/CD & DevOps', 0, Date.now() - phase15StartTime, errorMessage);
            }
            phaseResults.push({
                phase: 15,
                phaseName: 'CI/CD & DevOps',
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase15BResult = await this.runWithTimeout(() => phase15BCloudInfra.execute(), timeoutMs, 'Phase 15B: Cloud Infrastructure');
            if (!phase15BResult.success) {
                console.error(`ERROR Phase 15B failed: ${phase15BResult.error}`);
                console.log(`WARNING Continuing to next phase (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(15, 'Cloud Infrastructure', 0, Date.now() - phase15BStartTime, phase15BResult.error);
                }
                phaseResults.push({
                    phase: 15,
                ...
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(15, 'Cloud Infrastructure', phase15BResult.cloudInfraFindings.length, phase15BResult.executionTimeMs);
                }
                if (this.config.enableMemoryFlush && this.isHeavyPhase(15)) {
                    await this.flushMemory();
                }
            ...
            console.log(`WARNING Continuing to next phase (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(15, 'Cloud Infrastructure', 0, Date.now() - phase15BStartTime, errorMessage);
            }
            phaseResults.push({
                phase: 15,
                phaseName: 'Cloud Infrastructure',
        ...
        try {
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000; // 5 min default
            const phase15CResult = await this.runWithTimeout(() => phase15CContainerization.execute(), timeoutMs, 'Phase 15C: Containerization');
            if (!phase15CResult.success) {
                console.error(`ERROR Phase 15C failed: ${phase15CResult.error}`);
                console.log(`WARNING Continuing to final summary (system resilience)`);
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(15, 'Containerization', 0, Date.now() - phase15CStartTime, phase15CResult.error);
                }
                phaseResults.push({
                    phase: 15,
                ...
                if (this.config.enablePartialReports) {
                    await this.writePartialReport(15, 'Containerization', phase15CResult.containerizationFindings.length, phase15CResult.executionTimeMs);
                }
                if (this.config.enableMemoryFlush && this.isHeavyPhase(15)) {
                    await this.flushMemory();
                }
            ...
            console.log(`WARNING Continuing to final summary (system resilience)`);
            if (this.config.enablePartialReports) {
                await this.writePartialReport(15, 'Containerization', 0, Date.now() - phase15CStartTime, errorMessage);
            }
            phaseResults.push({
                phase: 15,
                phaseName: 'Containerization',
     ...
     */
    async runWithTimeout(fn, timeoutMs, context) {
     ...
     */
    performTimeoutCleanup(context) {
        console.error(`?? [Timeout Cleanup] Cleaning up resources after timeout in ${context}`);
        ...
        console.error(`?? [Timeout Cleanup] Cleanup completed for ${context}`);
    }
    /**
    ...
    async flushMemory() {
        try {
            ...
            if (typeof global.gc === 'function') {
                global.gc();
                console.log('  ���� Memory flushed (global.gc())');
            }
            else {
                ...
                console.log('  ���� Memory cleanup (object clearing)');
            }
        }
        catch (error) {
            // Memory flush failure should not halt execution
            console.warn('  ��ᴩ�  Memory flush failed:', error instanceof Error ? error.message : error);
        }
    }
    /**
     ...
     */
    async writePartialReport(phaseNumber, phaseName, findingsCount, executionTimeMs, error) {
        try {
            const reportPath = `${this.config.projectRoot}/qa-report.partial.md`;
            const timestamp = new Date().toISOString();
            const status = error ? '��� FAILED' : 'ԣ� PASSED';
            const reportEntry = `
...
`;
            ...
            console.log(`  ���� Partial report updated: ${reportPath}`);
        }
        catch (reportError) {
            // Report write failure should not halt execution
            console.warn('  ��ᴩ�  Failed to write partial report:', reportError instanceof Error ? reportError.message : reportError);
        }
    }
    /**
     ...
     */
    async runFixes() {
        console.log('���� Aegis QA - Atomic Fixes Mode\n');
        // Start global execution timer
        this.globalExecutionStartTime = Date.now();
        if (this.globalRuntimeLimitMs) {
            console.log(`[GlobalTimeout] Max runtime set to: ${this.formatElapsedTime(this.globalRuntimeLimitMs)}`);
        ...
        console.log('[Phase 16] Fix Strategy Generation - Intelligent Fix Strategy Planning');
        let phase16Result = null;
        try {
            ...
            const allFindings = Array.from(allViolations.values()).flat().map((v) => {
                const severity = v.severity === 'none' ? 'low' : v.severity;
                const riskLevel = severity === 'critical' ? 'risky' : severity === 'high' ? 'moderate' : 'safe';
                const type = 'clean-code';
                return {
                    id: v.id,
                    ...
                    riskLevel,
                    originalContent: '',
                    proposedContent: v.suggestion || '',
                    confidence: v.confidence,
                    blastRadius: 1,
                    category: 'atomic',
                    isCorePath: v.file.inCriticalPath,
                };
            });
            const phase16FixStrategyGeneration = new Phase16FixStrategyGeneration({
                projectRoot: this.getEffectiveProjectRoot(),
                thermalController: this.config.thermalController,
                statePersistence: this.config.statePersistence,
                currentState: this.config.currentState,
            ...
            phase16Result = await this.runWithTimeout(() => phase16FixStrategyGeneration.execute(), timeoutMs, 'Phase 16: Fix Strategy Generation');
            if (phase16Result.success) {
                console.log('[Phase 16] Fix Strategy Generation passed');
                console.log(`  Fix plan generated: ${phase16Result.fixPlan.length} strategies\n`);
            }
            else {
                console.warn(`[Phase 16] Failed: ${phase16Result.error}`);
                console.warn('[Phase 16] Continuing without fix strategy...\n');
            }
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.warn(`[Phase 16] Failed with exception: ${errorMessage}`);
            console.warn('[Phase 16] Continuing without fix strategy...\n');
        }
        // Phase 17: Multi-Fix Execution
        console.log('[Phase 17] Multi-Fix Execution - Batch Fix Application');
        let phase17Result = null;
        try {
            const phase17MultiFixExecution = new Phase17MultiFixExecution({
                projectRoot: this.getEffectiveProjectRoot(),
                thermalController: this.config.thermalController,
                statePersistence: this.config.statePersistence,
                currentState: this.config.currentState,
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000;
            phase17Result = await this.runWithTimeout(() => phase17MultiFixExecution.execute(), timeoutMs, 'Phase 17: Multi-Fix Execution');
            if (phase17Result.success) {
                console.log('[Phase 17] Multi-Fix Execution passed');
                console.log(`  Fixes applied: ${phase17Result.executionResult.fixesApplied}\n`);
            }
            else {
                console.warn(`[Phase 17] Failed: ${phase17Result.error}`);
                console.warn('[Phase 17] Rolling back to checkpoint...');
                await this.gitCheckpointManager.rollbackToLastCheckpoint();
                console.warn('[Phase 17] Continuing without fixes...\n');
            }
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.warn(`[Phase 17] Failed with exception: ${errorMessage}`);
            console.warn('[Phase 17] Rolling back to checkpoint...');
            await this.gitCheckpointManager.rollbackToLastCheckpoint();
            console.warn('[Phase 17] Continuing without fixes...\n');
        }
        // Phase 18: Post-Fix Validation
        console.log('[Phase 18] Post-Fix Validation - Global Integrity Check');
        let phase18Result = null;
        try {
            const multiFixResult = phase17Result?.success ? phase17Result.executionResult : undefined;
            const phase18PostFixValidation = new Phase18PostFixValidation({
                projectRoot: this.getEffectiveProjectRoot(),
                thermalController: this.config.thermalController,
                statePersistence: this.config.statePersistence,
                currentState: this.config.currentState,
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000;
            phase18Result = await this.runWithTimeout(() => phase18PostFixValidation.execute(), timeoutMs, 'Phase 18: Post-Fix Validation');
            if (phase18Result.success) {
                console.log('[Phase 18] Post-Fix Validation passed');
                console.log(`  Global integrity: ${phase18Result.globalIntegrityStatus}\n`);
                // Capture validation results for Phase 20 ROI calculation using contextStore
                if (!this.config.currentState.contextStore) {
                    this.config.currentState.contextStore = {};
                }
                if (phase18Result.fixesApplied !== undefined) {
                    this.config.currentState.contextStore.fixesApplied = phase18Result.fixesApplied;
                }
                if (phase18Result.riskReduction !== undefined) {
                    this.config.currentState.contextStore.riskReduction = phase18Result.riskReduction;
                }
            }
            else {
                console.warn(`[Phase 18] Failed: ${phase18Result.error}`);
                console.warn('[Phase 18] Continuing without validation...\n');
            }
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.warn(`[Phase 18] Failed with exception: ${errorMessage}`);
            console.warn('[Phase 18] Continuing without validation...\n');
        }
        // Phase 19: Incremental Review
        console.log('[Phase 19] Incremental Review - Git-Powered Target Selection');
        let phase19Result = null;
        try {
            const phase19IncrementalReview = new Phase19IncrementalReview({
                projectRoot: this.getEffectiveProjectRoot(),
                thermalController: this.config.thermalController,
                statePersistence: this.config.statePersistence,
                currentState: this.config.currentState,
            ...
            const timeoutMs = this.config.phaseTimeoutMs || 300000;
            phase19Result = await this.runWithTimeout(() => phase19IncrementalReview.execute(), timeoutMs, 'Phase 19: Incremental Review');
            if (phase19Result.success) {
                console.log('[Phase 19] Incremental Review passed');
                console.log(`  Files selected for audit: ${phase19Result.selectedFiles.length}\n`);
            }
            else {
                console.warn(`[Phase 19] Failed: ${phase19Result.error}`);
                console.warn('[Phase 19] Continuing without incremental review...\n');
            }
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.warn(`[Phase 19] Failed with exception: ${errorMessage}`);
            console.warn('[Phase 19] Continuing without incremental review...\n');
        }
     ...
     */
    async runIncrementalReview() {
        console.log('���� Aegis QA - Incremental Review Mode');
        console.log(`���� Project Root: ${this.config.projectRoot}\n`);
        // Start global execution timer
        this.globalExecutionStartTime = Date.now();
        if (this.globalRuntimeLimitMs) {
        ...
        console.log('��ᴩ�  Incremental review not yet implemented (skeleton)');
        // Check global timeout before returning
        if (this.hasGlobalTimeoutExceeded()) {
    ...
    async compareReports() {
        console.log('���� Aegis QA - Compare Reports Mode\n');
