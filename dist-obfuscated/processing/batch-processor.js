...
export class BatchProcessor {
     ...
     * ```
     */
    async processFiles(files, processorFn, currentState) {
        ...
        for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
            ...
            const batchResult = await this.processBatch(batchFiles, processorFn, batchNumber, totalBatches, currentState);
     ...
     */
    async processBatch(files, processorFn, batchNumber, totalBatches, currentState) {
        ...
        for (const filePath of files) {
            try {
                ...
                const alreadyProcessed = currentState.files.find((f) => f.filePath === filePath && f.processed);
                if (alreadyProcessed) {
                ...
                await this.config.statePersistence.recordFile(filePath, result.success, result.error, currentState);
                if (result.success) {
                    successCount++;
                    if (result.findings) {
                ...
                await this.config.statePersistence.recordFile(filePath, false, errorMessage, currentState);
                failureCount++;
     ...
     * ```
     */
    adjustBatchSize(performanceFactor) {
