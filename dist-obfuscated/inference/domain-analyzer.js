...
export class DomainAnalyzer {
     ...
     */
    constructor(config, secretManager, databaseIntrospector) {
        const defaultConfig = {
    ...
    async analyze() {
        ...
        try {
            ...
            const overallConfidence = this.calculateOverallConfidence(uniqueEntities, uniqueRelationships, warnings);
            // Determine analysis method
            const method = this.config.useDatabase && !this.secretManager.isMockMode()
                ? 'hybrid'
                : 'static';
            const domainMap = {
        ...
        try {
            ...
            for (const table of tables || []) {
                ...
                if (columns) {
                    ...
                    entity.fields = columns.map((c) => c.column_name);
                }
     ...
     */
    parseSQLSchema(content, filePath) {
        const entities = [];
     ...
     * @returns Entity[] - Extracted entities
     */
    parsePrismaSchema(content, filePath) {
        const entities = [];
     ...
     * @returns Entity[] - Extracted entities
     */
    parseTypeScriptTypes(content, filePath) {
        const entities = [];
     ...
     */
    parseServerActions(content, filePath) {
        const actions = [];
     ...
     */
    extractFieldsFromSQL(content, tableName) {
        const fields = [];
     ...
     * @returns string[] - Field names
     */
    extractFieldsFromPrisma(modelBody) {
        const fields = [];
     ...
     * @returns string[] - Field names
     */
    extractFieldsFromTypeScript(typeBody) {
        const fields = [];
     ...
     */
    extractEntitiesFromFunction(content, functionName) {
        const entities = [];
        ...
        const functionRegex = new RegExp(`(?:export\\s+)?(?:async\\s+)?function\\s+${functionName}\\s*\\([^)]*\\)\\s*\\{([^}]*(?:\\{[^}]*\\}[^}]*)*)\\}`, 's');
     ...
     * @returns Action type
     */
    inferActionType(functionName, content) {
     ...
     */
    inferCriticalPaths(entities, serverActions) {
        const criticalPaths = [];
     ...
     */
    calculateOverallConfidence(entities, relationships, warnings) {
        if (entities.length === 0) {
            return 0;
        }
     ...
     */
    applyMapOverride(domainMap, override) {
        // Override entity properties
        if (override.entities) {
            for (const overrideEntity of override.entities) {
                const existingIndex = domainMap.entities.findIndex((e) => e.name.toLowerCase() === overrideEntity.name?.toLowerCase());
        ...
        if (override.criticalPaths) {
            for (const overridePath of override.criticalPaths) {
                const existingIndex = domainMap.criticalPaths.findIndex((cp) => cp.name.toLowerCase() === overridePath.name?.toLowerCase());
