...
import { FileWhitelist } from '../core/file-whitelist.js';
...
export class Phase11AtomicFixes {
    ...
    constructor(config) {
        ...
        const operationGuardConfig = config.operationGuardConfig || {
            allowRead: true,
            allowWrite: true,
            allowDelete: false,
        ...
        const fileWhitelistConfig = config.fileWhitelistConfig || {
     ...
     */
    isDestructiveOperation(fixResult) {
     ...
     */
    async showInteractiveConfirmation(remediationResult, forceConfirmation = false) {
        ...
        if (previewFixes.length > 0) {
            ...
            for (const fix of previewFixes) {
                ...
                if (fix.originalContent && fix.newContent) {
                    const fileDiff = this.diffGenerator.generateFileDiff(path.basename(fix.filePath), fix.originalContent, fix.newContent);
     ...
     */
    async showPerFixApproval(fixResult) {
        console.log(`\n🔍 Fix: ${fixResult.fixId}`);
        ...
        if (fixResult.originalContent && fixResult.newContent) {
            console.log('\n📝 Diff Preview:');
            const fileDiff = this.diffGenerator.generateFileDiff(path.basename(fixResult.filePath), fixResult.originalContent, fixResult.newContent);
            const diffLines = fileDiff.unifiedDiff.split('\n').slice(0, 10);
            console.log(diffLines.join('\n'));
     ...
     */
    async showBatchDiffPreview(remediationResult) {
        const fixCount = remediationResult.fixResults.length;
        const fileCount = new Set(remediationResult.fixResults.map(f => f.filePath)).size;
        console.log('\n' + '='.repeat(60));
        ...
        for (const [filePath, fixes] of fixesByFile.entries()) {
            ...
            if (firstFix.originalContent && firstFix.newContent) {
                const fileDiff = this.diffGenerator.generateFileDiff(path.basename(filePath), firstFix.originalContent, firstFix.newContent);
     ...
     */
    calculateMetrics(remediationResult) {
        ...
        const operationGuardBlocks = remediationResult.fixResults.filter(f => f.error?.includes('Operation guard blocked')).length;
        // Count file whitelist blocks (from error messages)
        const fileWhitelistBlocks = remediationResult.fixResults.filter(f => f.error?.includes('File whitelist blocked')).length;
        return {
     ...
     */
    logAuditEvent(eventType, description, fixId, filePath, metadata) {
     ...
     * @returns Promise<void>
     */
    async createAutoBackup(filePaths) {
     ...
     * @returns Promise<void>
     */
    async createPhysicalBackup(filePaths) {
     ...
     */
    getCorePathFiles(analysisResults) {
        const corePathFiles = new Set();
     ...
     */
    async applyI18nA11yFixes(remediationResult, corePathFiles) {
     ...
     */
    async fixMissingAltAttributes(filePath, content, isCorePath) {
     ...
     * @returns Promise<FixApplicationResult | null>
     */
    async fixMissingAriaLabels(filePath, content, isCorePath) {
     ...
     * @param analysisResults - Analysis results from previous phases
     */
    async applyEnvironmentFixes(remediationResult, analysisResults) {
     ...
     * @param corePathFiles - Core Path files
     */
    async applyCleanCodeFixes(remediationResult, corePathFiles) {
     ...
     * @returns Promise<FixApplicationResult | null>
     */
    async refactorToOptionsObject(filePath, content, isCorePath) {
     ...
     */
    async generatePatchFile(fixId, filePath, originalContent, newContent) {
     ...
     */
    generateUnifiedDiff(filePath, originalContent, newContent) {
     ...
     */
    generateFixId(fixType, filePath) {
     ...
     * @returns Promise<void>
     */
    async validateFixes(remediationResult, phaseNumber) {
        try {
     ...
     */
    extractModifiedLines(originalContent, newContent) {
        const originalLines = originalContent.split('\n');
        const newLines = newContent.split('\n');
        const modifiedLines = [];
        for (let i = 0; i < Math.max(originalLines.length, newLines.length); i++) {
     ...
     */
    checkCollision(filePath, modifiedLines, existingFixes) {
     ...
     */
    addTraceabilityComment(content, fixId, originalViolationId) {
     ...
     */
    async writePartialReport(remediationResult) {
        try {
...
//# sourceMappingURL=phase-11-atomic-fixes.js.map