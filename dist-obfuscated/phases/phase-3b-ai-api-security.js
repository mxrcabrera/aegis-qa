...
export class Phase3BAIAPIIntegration {
    ...
    async analyzeAIAPIIntegrations(sourceFiles) {
        const findings = [];
    ...
    analyzeFileForAIAPI(filePath, content) {
        const findings = [];
    ...
    detectAPIProvider(line) {
        const lowerLine = line.toLowerCase();
        if (lowerLine.includes('openai') || lowerLine.includes('gpt-'))
            return 'openai';
        if (lowerLine.includes('anthropic') || lowerLine.includes('claude'))
            return 'anthropic';
        if (lowerLine.includes('cohere'))
            return 'cohere';
        if (lowerLine.includes('huggingface') || lowerLine.includes('transformers'))
            return 'huggingface';
        if (lowerLine.includes('ai.') || lowerLine.includes('llm') || lowerLine.includes('completion'))
            return 'custom';
        return null;
    }
    checkPromptInjection(filePath, line, content, apiProvider) {
        const findings = [];
        const patterns = [
    ...
    checkKeyExposure(filePath, line, content, apiProvider) {
        const findings = [];
        const patterns = [
    ...
    checkCostLimits(filePath, line, content, apiProvider) {
        const findings = [];
    ...
    checkOutputValidation(filePath, line, content, apiProvider) {
        const findings = [];
...
//# sourceMappingURL=phase-3b-ai-api-security.js.map