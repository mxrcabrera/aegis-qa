...
export class Phase13I18nL10n {
    ...
    async execute() {
        ...
        try {
            ...
            const corePathFiles = new Set(phase2Results?.corePathFiles || []);
    ...
    async detectI18nLibrary() {
        ...
        for (const pattern of translationPatterns) {
            ...
            if (fs.existsSync(localeDir)) {
                ...
                const findTranslationFiles = (dir) => {
                    const entries = fs.readdirSync(dir, { withFileTypes: true });
                    for (const entry of entries) {
                        const fullPath = path.join(dir, entry.name);
                        if (entry.isDirectory()) {
     ...
     */
    async analyzeFile(filePath, corePathFiles, i18nLibrary, businessDomain) {
        const findings = [];
        ...
        if (isUIFile) {
            ...
            for (const stringInfo of hardcodedStrings) {
                ...
                let severity = isCorePath ? 'high' : 'medium';
     ...
     */
    detectHardcodedStrings(content, _filePath) {
        const results = [];
     ...
     */
    detectLocaleHardcoding(content) {
        const results = [];
        const lines = content.split('\n');
     ...
     */
    async writePartialReport(findings, i18nLibrary, criticalFindings, highSeverityFindings) {
        try {
...
`;
     ...
     */
    generateFindingId(filePath, line, type) {
...
//# sourceMappingURL=phase-13a-i18n-l10n.js.map