...
export class Phase0Setup {
     ...
     */
    async writePartialReport(setupResults) {
        try {
...
`;
     ...
     */
    detectLockfileType(projectRoot) {
     ...
     */
    async checkLockfileConsistency(_lockfileType, projectRoot) {
     ...
     * @returns Promise<SyntaxCheckResult>
     */
    async basicSyntaxCheckFallback() {
        const projectRoot = this.config.projectRoot;
        const errorFiles = [];
        let filesChecked = 0;
        const fileFilter = this.config.fileFilter || new FileFilter();
...
//# sourceMappingURL=phase-0-setup.js.map