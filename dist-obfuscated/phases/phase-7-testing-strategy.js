...
export class Phase7TestingStrategy {
     ...
     */
    analyzeTestCoverage(sourceFiles, testFiles, criticalModules) {
        const findings = [];
     ...
     */
    async analyzeTestQuality(testFilePath, criticalModules) {
        const findings = [];
        try {
            ...
            const mockLines = lines.filter(line => line.includes('mock(') ||
                line.includes('jest.fn(') ||
                line.includes('vi.fn(') ||
                line.includes('mockReturnValue') ||
                line.includes('mockResolvedValue')).length;
            const executionLines = lines.filter(line => line.includes('expect(') ||
                line.includes('assert.') ||
                line.includes('it(') ||
                line.includes('test(')).length;
     ...
     */
    analyzeCrossPhaseCoverageGap(sourceFiles, testFiles, complexityScores, criticalModules) {
        const findings = [];
        // Create a map of test files for quick lookup
        const testFileMap = new Map();
        for (const testFile of testFiles) {
     ...
     */
    analyzeIntegrationTests(testFiles, isFintech) {
        const findings = [];
...
//# sourceMappingURL=phase-7-testing-strategy.js.map