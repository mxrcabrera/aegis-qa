...
export class Phase6APIContracts {
    ...
    async execute() {
        ...
        try {
            ...
            const sensitiveFields = new Set(securityFindings
                .filter((f) => f.type === 'sensitive-data' || f.type === 'secret')
                .map((f) => f.description.toLowerCase()));
            // Cross-Phase PII Leak Prevention: Track files with Sensitive Data Leak
            const piiLeakFiles = new Set(securityFindings
                .filter((f) => f.type === 'sensitive-data' && f.description.toLowerCase().includes('console.log'))
                .map((f) => f.filePath));
     ...
     */
    async analyzeFile(filePath, isSaaS, isFintech, sensitiveFields, piiLeakFiles) {
        const findings = [];
        try {
     ...
     */
    analyzeEndpointIntegrity(filePath, content, fileHash, isSaaS, isFintech) {
        const findings = [];
        // Detect endpoints without versioning
        const unversionedPattern = /['"`]\/api\/(?!v\d+)[^'"`]*['"`]/g;
        let unversionedMatch;
     ...
     */
    analyzeContractConsistency(filePath, content, fileHash, sensitiveFields) {
        const findings = [];
     ...
     */
    analyzeDocumentationGap(filePath, content, fileHash) {
        const findings = [];
     ...
     */
    analyzeInputValidation(filePath, content, fileHash, isCriticalModule) {
        const findings = [];
     ...
     */
    analyzeTypeValidation(filePath, content, fileHash, isCriticalModule) {
        const findings = [];
     ...
     */
    async writePartialReport(result, domain) {
        try {
...
//# sourceMappingURL=phase-6-api-contracts.js.map