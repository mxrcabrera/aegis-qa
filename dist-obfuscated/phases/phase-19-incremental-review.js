...
export class Phase19IncrementalReview {
     ...
     */
    checkDiffPressure(modifiedFileCount) {
     ...
     */
    async buildFileAuditStatusMap(modifiedFiles) {
     ...
     */
    async performCorePathDependencyTracing(fileStatusMap) {
     ...
     */
    async buildReverseDependencyMapBatched() {
        ...
        await batchProcessor.processFiles(sourceFiles, async (sourceFile) => {
            try {
                const content = fs.readFileSync(sourceFile, 'utf-8');
     ...
     */
    async performHashValidationDoubleCheck(fileStatusMap) {
     ...
     */
    async updateSavedFileHashes(fileStatusMap) {
     ...
     */
    async getUntrackedCorePathFiles() {
        const untrackedCorePathFiles = [];
        try {
            // Get untracked files
     ...
     */
    performAuditSkip(fileStatusMap) {
     ...
     */
    selectFilesForAudit(fileStatusMap) {
        const selectedFiles = [];
        for (const [filePath, status] of fileStatusMap.entries()) {
            if (status.shouldAudit) {
...
//# sourceMappingURL=phase-19-incremental-review.js.map