...
export class Phase3FWebhookSecurity {
    ...
    async checkSignatureVerification() {
        const findings = [];
        ...
        for (const filePath of sourceFiles) {
            try {
                ...
                if (isWebhookHandler) {
                    ...
                    const hasSignatureCheck = content.includes('verify') ||
        ...
        for (const filePath of sourceFiles) {
            try {
                ...
                lines.forEach((line, index) => {
                    ...
                    const isIntegrationHandler = line.toLowerCase().includes('webhook') ||
                    ...
                    if (isIntegrationHandler) {
                        const integrationType = this.detectIntegrationType(content);
                        const hasIdempotency = content.includes('idempotency') ||
    ...
    async checkExposedWebhookSecrets() {
        const findings = [];
        const sourceFiles = this.findSourceFiles();
        for (const filePath of sourceFiles) {
            try {
    ...
    async checkRetryLogic() {
        ...
        for (const filePath of sourceFiles) {
            try {
                ...
                const hasIntegration = content.toLowerCase().includes('stripe') ||
    ...
    detectIntegrationType(content) {
        const lowerContent = content.toLowerCase();
        if (lowerContent.includes('stripe') || lowerContent.includes('payment_intent'))
            return 'stripe';
        if (lowerContent.includes('github') || lowerContent.includes('repository'))
            return 'github';
        if (lowerContent.includes('slack') || lowerContent.includes('webhook.slack.com'))
            return 'slack';
        return 'custom';
    }
    findSourceFiles() {
...
//# sourceMappingURL=phase-3f-webhook-security.js.map