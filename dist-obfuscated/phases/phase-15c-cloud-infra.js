...
export class Phase15BCloudInfra {
    ...
    async execute() {
        ...
        try {
            ...
            const allFindings = this.generateFindingsFromAudit(cloudInfraAudit);
     ...
     */
    analyzeTerraformFile(filePath, result) {
        try {
     ...
     * @param result - Cloud infrastructure audit result
     */
    analyzeCloudFormationFile(filePath, result) {
        try {
            const content = fs.readFileSync(filePath, 'utf-8');
            const lines = content.split('\n');
            for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
...
//# sourceMappingURL=phase-15c-cloud-infra.js.map