...
export class Phase4Database {
     ...
     */
    async scanDatabaseFiles(ormType) {
     ...
     */
    async analyzeFile(filePath, hasSqlInjectionRisks, isFintech) {
        const findings = [];
        try {
     ...
     */
    analyzeSchema(filePath, content, fileHash, isFintech) {
        const findings = [];
        ...
        const keyFieldPatterns = [
            { pattern: /\b(email)\s*:/g, name: 'email', severity: 'high' },
            { pattern: /\b(userId)\s*:/g, name: 'userId', severity: 'high' },
            { pattern: /\b(slug)\s*:/g, name: 'slug', severity: 'medium' },
            { pattern: /\b(username)\s*:/g, name: 'username', severity: 'medium' },
            { pattern: /\b(externalId)\s*:/g, name: 'externalId', severity: 'medium' },
        ];
        // Add Fintech-specific fields if in Fintech domain
        if (isFintech) {
            keyFieldPatterns.push({ pattern: /\b(amount)\s*:/g, name: 'amount', severity: 'high' }, { pattern: /\b(currency)\s*:/g, name: 'currency', severity: 'high' }, { pattern: /\b(status)\s*:/g, name: 'status', severity: 'high' });
        }
     ...
     */
    analyzeQueries(filePath, content, fileHash) {
        const findings = [];
        ...
        while ((match = findManyPattern.exec(content)) !== null) {
            ...
            const inSensitiveContext = sensitiveTables.some(table => content.slice(0, matchIndex).toLowerCase().includes(table));
     ...
     */
    analyzeStringConcatenation(filePath, content, fileHash) {
        const findings = [];
     ...
     * @param ormType - Detected ORM type
     */
    async writePartialReport(result, ormType) {
        try {
...
//# sourceMappingURL=phase-4-database.js.map