...
export class Phase10EnvCICD {
    ...
    analyzeCICDConfiguration() {
        ...
        if (criticalVars.length > 0 && (hasVercel || hasGitHubWorkflows)) {
            ...
            if (hasVercel)
                deploymentFiles.push(vercelPath);
            ...
            for (const deployFile of deploymentFiles) {
                try {
                    ...
                    const missingInDeploy = criticalVars.filter((v) => !content.includes(v));
     ...
     */
    analyzeSecurityCheck(securityFindings) {
        const findings = [];
        // Check for secrets or private keys in repo (cross-reference with Phase 3)
        const secretFindings = securityFindings.filter((f) => f.type === 'secret-leak' || f.type === 'api-key-exposure' || f.type === 'hardcoded-secret');
...
//# sourceMappingURL=phase-10-env-cicd.js.map