...
export class Phase20IntelligentReportComparison {
     ...
     */
    async performDiffBasedReport() {
        console.log('INFO Performing Diff-Based Report...');
        const comparison = { critical: 0, high: 0, medium: 0, low: 0 };
        try {
    ...
    async detectRegressions() {
        ...
        for (const previousFinding of previousFindings) {
            if (previousFinding.status === 'FIXED') {
                const currentFinding = currentFindings.find((f) => f.filePath === previousFinding.filePath && f.type === previousFinding.type);
     ...
     */
    async saveThermalProfile(profile) {
        try {
    ...
    async detectHistoricalDrift() {
        ...
        for (const historyFile of historyFiles) {
            try {
                ...
                const totalErrors = (historyData.severityComparison?.critical || 0) +
...
//# sourceMappingURL=phase-20-intelligent-report-comparison.js.map