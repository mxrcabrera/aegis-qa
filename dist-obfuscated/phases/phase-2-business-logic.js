...
export class Phase2BusinessLogic {
    ...
    async execute() {
        ...
        try {
            ...
            console.log(`PASSED Phase 2 Complete`);
            console.log(`  Domain: ${businessProfile.domain}`);
            console.log(`  Confidence: ${businessProfile.confidence}%`);
            console.log(`  Stack: ${businessProfile.stack.length} dependencies`);
            console.log(`  Critical modules: ${businessProfile.criticalModules.length}`);
            console.log(`  Risk findings: ${businessProfile.riskFindings.length}`);
            console.log(`  Recommended focus: ${businessProfile.recommendedFocus.join(', ')}\n`);
            return {
                success: true,
                businessProfile,
                executionTimeMs,
     ...
     */
    combineDomainDetections(depsResult, readmeResult) {
     ...
     */
    async crossReferenceWithPhase1(corePaths) {
        const riskFindings = [];
        ...
        if (!phase1Results || !phase1Results.fileScores) {
            console.log('  Phase 1 results not found, skipping cross-reference');
            return riskFindings;
        }
     ...
     */
    generateRecommendedFocus(domain, riskFindings) {
        const focus = [];
    ...
    detectSelfAudit() {
        ...
        try {
            ...
            const isAegisQA = packageName.includes('aegis') ||
     ...
     */
    generatePriorityPhase(domain, isSelfAudit) {
     ...
     */
    generateUntouchableFolders(domain, corePaths, isSelfAudit) {
        const untouchable = [];
     ...
     */
    calculateSensitivityLevel(domain, coreFlow) {
     ...
     */
    generateBusinessUnderstanding(domain, coreFlow, isSelfAudit) {
        if (isSelfAudit) {
     ...
     */
    async writePartialReport(businessProfile) {
        try {
...
//# sourceMappingURL=phase-2-business-logic.js.map