...
class DomainInference {
     ...
     */
    parsePrismaSchema(content, _filePath) {
     ...
     */
    parseTypeScriptSchema(content, _filePath) {
        ...
        for (const entity of entities) {
            for (const field of entity.fields) {
                ...
                if (field.type.includes("[]")) {
                    ...
                    const targetEntity = entities.find((e) => e.name.toLowerCase() === arrayType.toLowerCase());
                    if (targetEntity) {
                        entity.relationships.push({
                            type: "one-to-many",
                            target: targetEntity.name,
    ...
    inferBusinessRules(entities) {
        for (const entity of entities) {
            ...
            if (entity.fields.some((f) => f.name === "password")) {
                rules.push(`${entity.name} requiere password con requisitos de seguridad`);
            }
            if (entity.fields.some((f) => f.name === "status" || f.name === "estado")) {
                rules.push(`${entity.name} tiene ciclo de vida de estados`);
            }
            if (entity.fields.some((f) => f.name.includes("created_at") || f.name.includes("createdAt"))) {
                rules.push(`${entity.name} tiene tracking de creación`);
            }
            if (entity.fields.some((f) => f.name.includes("updated_at") || f.name.includes("updatedAt"))) {
                rules.push(`${entity.name} tiene tracking de actualización`);
            }
            if (entity.fields.some((f) => f.name === "deleted_at" || f.name === "deletedAt")) {
                rules.push(`${entity.name} usa soft delete`);
            }
            // Reglas basadas en nombre de entidad
            if (entity.name.toLowerCase().includes("user") ||
            ...
            if (entity.name.toLowerCase().includes("order") ||
                ...
                rules.push(`${entity.name} tiene workflow de estados (pending, confirmed, shipped, delivered)`);
            }
            if (entity.name.toLowerCase().includes("payment") ||
                entity.name.toLowerCase().includes("pago")) {
                rules.push(`${entity.name} requiere integración con gateway de pagos`);
                rules.push(`${entity.name} maneja estados de transacción (pending, completed, failed, refunded)`);
            }
     ...
     */
    generateDomainSummary(entities) {
    ...
    generateInferences(entities) {
        ...
        if (entities.some((e) => e.name.toLowerCase().includes("order") ||
        ...
        if (entities.some((e) => e.name.toLowerCase().includes("payment") ||
            e.name.toLowerCase().includes("pago"))) {
            inferences.push("Procesamiento de pagos online");
        }
        if (entities.some((e) => e.name.toLowerCase().includes("reservation") ||
        ...
        if (entities.length > 10) {
            inferences.push("Dominio complejo con múltiples entidades interconectadas");
        }
...
//# sourceMappingURL=domain-inference.js.map