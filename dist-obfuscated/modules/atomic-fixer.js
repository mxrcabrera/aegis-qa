...
export class AtomicFixer {
    ...
    runTests;
    testCommand;
    testBaseline; // Test name -> passed/failed before fixes
    /**
     ...
     */
    generateDeterministicId(content, file, line) {
     ...
     */
    calculateRiskLevel(fixType, originalContent, proposedContent, isCorePath, impactScore) {
    ...
    constructor(projectRoot, interactiveMode = true, dryRun = false, maxRisk = 'safe', minConfidence = 0.8, runTests = false, testCommand) {
     ...
     */
    async runTestCommand(command, timeoutMs = 60000, filePath) {
        ...
        return new Promise((resolve) => {
            ...
            exec(optimizedCommand, { cwd: this.projectRoot }, (error, stdout, stderr) => {
                ...
                const testLines = output.split('\n').filter(line => line.includes('PASS') || line.includes('FAIL') || line.includes('✓') || line.includes('✗'));
     ...
     */
    async validateTestsAfterFix(fix) {
        const testCommand = this.getTestCommand();
        if (!testCommand) {
     ...
     */
    async runFixes(violations, domainModel) {
        const results = {
            atomicFixes: [],
     ...
     */
    applyFixesWithMicroPasses(fileGroups) {
        const allFixes = [];
     ...
     */
    processFixesForFile(_file, fixes) {
     ...
     */
    wouldAffectRemainingFixes(appliedFix, remainingFixes, _offset) {
     ...
     * Generate fixes based on violations
     */
    async generateFixes(violations, _domainModel) {
        const fixes = [];
     ...
     */
    async generateI18nFix(violation) {
        ...
        const riskLevel = this.calculateRiskLevel(violation.rule === 'missing-alt' ? 'i18n' : 'a11y', line, proposedContent, this.isCorePath(filePath), impactScore);
        return {
            id: fixId,
     ...
     */
    async generateEnvironmentFix(violation) {
        const violationId = violation.id || '';
     ...
     */
    async generateCleanCodeFix(violation) {
        const filePath = violation.file?.path || '';
        const violationId = violation.id || '';
        if (!filePath || !fs.existsSync(filePath)) {
            return null;
     ...
     */
    async generatePatch(fix, patchPath) {
     ...
     */
    async getTscErrors(filePath, tscArgs = ['--noEmit', '--pretty', 'false']) {
        const { spawn } = await import('child_process');
        return new Promise((resolve) => {
     ...
     */
    countNewErrors(baselineErrors, newErrors) {
     ...
     */
    async rollbackFix(fix, backupPath) {
        try {
     ...
     * Add Fix ID traceability comment
     */
    async addTraceabilityComment(fix) {
        if (!fix.violationId) {
            return;
        }
        try {
...
//# sourceMappingURL=atomic-fixer.js.map