...
class DBSeeder {
        ...
        try {
            ...
            for (const student of this.testData.students) {
                const { data, error } = await client
                    ...
                    .single();
                if (error) {
                    console.warn(`Failed to insert student ${student.id}:`, error.message);
                }
                else if (data) {
     ...
     */
    async createReservation(estudianteId, claseId) {
        try {
            const client = await this.getAdminClient();
            const reserva = {
    ...
    async cleanup() {
        try {
            ...
            console.log(`  - Reservations deleted: ${result.deletedRecords.reservations}`);
            return result;
        }
        catch (error) {
            return {
