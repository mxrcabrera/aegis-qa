...
export class CodeReader {
    ...
    currentBatchSize;
    originalBatchSize;
    criticalPathsByDirectory = new Map();
    /**
        ...
        if (this.config.thermalController) {
            ...
            if (!tempReading.isSafe) {
                ...
                if (tempReading.category === 'warning' && this.currentBatchSize > this.originalBatchSize / 2) {
                    ...
                    console.warn(`[CodeReader] Thermal WARNING: Reducing batch size to ${this.currentBatchSize} (from ${this.originalBatchSize})`);
                }
        ...
        for (const batch of fileBatches) {
            ...
            if (this.config.thermalController && batch !== fileBatches[fileBatches.length - 1]) {
                ...
                if (!tempReading.isSafe) {
                    if (tempReading.category === 'warning' && this.currentBatchSize > this.originalBatchSize / 2) {
                        this.currentBatchSize = Math.max(1, Math.floor(this.currentBatchSize / 2));
                        console.warn(`[CodeReader] Thermal WARNING between batches: Reducing batch size to ${this.currentBatchSize}`);
                    }
                    else if (tempReading.category === 'critical' && this.currentBatchSize > 1) {
                        this.currentBatchSize = 1;
     ...
     */
    async processBatch(files, appPath) {
        const nodes = [];
     ...
     */
    async detectServerActions(filePath) {
        try {
     ...
     */
    determineCriticalPath(filePath) {
     ...
     */
    storeCriticalPathForInheritance(filePath, criticalPath) {
     ...
     */
    inheritCriticalityFromLocalImports(filePath) {
        try {
     ...
     */
    applyInheritanceToComponents(nodes) {
     ...
     */
    getFilesInCriticalPath(criticalPathName) {
