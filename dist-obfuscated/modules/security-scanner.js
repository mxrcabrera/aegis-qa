...
export class SecurityScanner {
    ...
    async scan() {
        ...
        if (this.config.thermalController) {
            ...
            if (!tempReading.isSafe) {
                console.warn(`[SecurityScanner] GPU temperature unsafe (${tempReading.current}°C), applying cooldown`);
     ...
     */
    async scanRLSPolicies(filePath) {
        try {
     ...
     */
    checkServiceRoleKey(line, lineNumber, metadata, fullContent) {
        const violations = [];
        ...
        for (const pattern of patterns) {
            ...
            if (match) {
                ...
                if (!isServerAction) {
                    ...
                    const violation = this.createViolation('security', 'critical', lineNumber, metadata, `Service Role Key detected in client code or unprotected file. This key bypasses RLS and should NEVER be used on the client side. Use ANON key instead.`, 'security-service-role-abuse', true);
                    ...
                    const violation = this.createViolation('security', 'medium', lineNumber, metadata, `Service Role Key detected in Server Action. This is technically safe, but consider if ANON key with proper RLS would be more appropriate.`, 'security-service-role-server-action', true);
                    // Add business context
                    violation.message = this.addBusinessContext(violation.message, metadata);
                    violations.push(violation);
                    // Track usage for reporting
    ...
    checkMissingRLSPolicies() {
        ...
        for (const tableName of criticalTables) {
            ...
            if (!hasSelectOrAll || policies.length === 0) {
                ...
                const violation = this.createViolation('security', 'high', 1, metadata, `Atención: La tabla "${tableName}" es crítica pero no detecto políticas de RLS activas en Supabase. Esta tabla debería tener políticas de SELECT o ALL para proteger los datos.`, 'security-missing-rls-policy', true);
                // Add business context
                violation.message = this.addBusinessContext(violation.message, metadata);
                violations.push(violation);
            }
     ...
     */
    createViolation(type, severity, lineNumber, metadata, message, rule, autoFixable) {
        return {
