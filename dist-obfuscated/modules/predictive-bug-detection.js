...
export class PredictiveBugDetection {
     ...
     */
    async detect(dirtyDataZones = [], heavyComputationFiles = []) {
        const results = {
            issues: [],
            summary: {
                critical: 0,
     ...
     */
    async analyzeFile(filePath, dirtyDataZones, heavyComputationFiles) {
        const issues = [];
     ...
     */
    detectSilentKiller(line, lines, index, isCorePath, filePath) {
     ...
     */
    detectRaceCondition(line, lines, index, isCorePath, isHeavyComputation) {
        // await inside .map()
     ...
     */
    detectImplicitNulls(line, filePath, dirtyDataZones, isCorePath) {
     ...
     */
    detectHighFragility(line, lines, index, isCorePath) {
     ...
     */
    detectStateMutation(line, filePath, isCorePath) {
     ...
     */
    detectExplosiveCombos(issues, patterns, filePath) {
