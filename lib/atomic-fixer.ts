...
export class AtomicFixer {
  ...
  async runFixes(violations: any[], domainModel?: any): Promise<RemediationResults> {
    ...
    await fs.ensureDir(this.diffsPath);
  ...
  private async applyFix(fix: Fix): Promise<FixResult> {
    ...
    try {
      ...
      if (fix.requiresConfirmation && this.interactiveMode) {
        ...
        if (!shouldApply) {
          result.error = 'User declined to apply fix';
          return result;
        }
      }
      ...
      if (fix.isCorePath && this.interactiveMode) {
        const shouldApply = await confirm({
          message: `⚠️  This fix is in Core Path. Apply anyway?`
        });

        if (!shouldApply) {
          result.error = 'User declined to apply Core Path fix';
          return result;
        }
      }
  ...
  private async writeFix(fix: Fix): Promise<void> {
    if (!await fs.pathExists(fix.file)) {
      // Create new file
      await fs.ensureFile(fix.file);
      await fs.writeFile(fix.file, fix.proposedContent, 'utf-8');
      return;
    }
  ...
  private async validateFix(fix: Fix): Promise<boolean> {
    ...
    if (fix.file.endsWith('.ts') || fix.file.endsWith('.tsx')) {
      try {
        ...
        await fs.readFile(fix.file, 'utf-8');
        return true;
      } catch {
        return false;
      }
  ...
  private async createBackup(fix: Fix): Promise<string> {
    ...
    if (await fs.pathExists(fix.file)) {
      await fs.copy(fix.file, backupPath);
    } else {
  ...
  private async rollbackFix(fix: Fix, backupPath: string): Promise<void> {
    try {
      if (await fs.pathExists(backupPath)) {
        ...
        await fs.writeFile(fix.file, backupContent, 'utf-8');
      } else {
        // File didn't exist originally, delete it
        if (await fs.pathExists(fix.file)) {
          await fs.remove(fix.file);
        }
      }
    } catch (error) {
  ...
  private async addTraceabilityComment(fix: Fix): Promise<void> {
    ...
    try {
      ...
      await fs.appendFile(fix.file, traceabilityComment, 'utf-8');
    } catch (error) {
