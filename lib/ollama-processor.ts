...
class OllamaProcessor {
  private static instance: OllamaProcessor;
  private model: string = "llama3.2:3b";
  private stats: ProcessingStats;
  private logAPB: string[] = [];
   ...
   */
  async processProject(projectPath: string): Promise<void> {
    console.log("");
          ...
          new Promise((_, reject) =>
            setTimeout(
              () =>
                reject(
                  new Error(
                    `TIMEOUT: ${file} - Ollama no respondió en 30 segundos`,
                  ),
                ),
              30000,
            ),
  ...
  private showProgressBar(): void {
    ...
    process.stdout.write(
      `[${bar}] ${progress.toFixed(1)}% | ${this.stats.filesProcessed}/${this.stats.filesTotal} | ${path.basename(this.stats.currentFile)}`,
    );
  }
   ...
   */
  private async processFileWithOllama(filePath: string): Promise<void> {
    try {
...
Mantené toda la funcionalidad intacta y asegurate que el código sea compilable.`;
      ...
      if (response.code && response.code !== originalContent) {
        ...
        if (this.isValidCode(response.code)) {
          ...
          console.warn(
            `Invalid code generated for ${filePath}, skipping write`,
          );
        }
      }
    } catch (error) {
  ...
  private isValidCode(code: string): boolean {
    try {
      ...
      if (code.length < 20) return false; // Too short
...
Devuelve el código optimizado:`;
      ...
      return new Promise<OllamaResponse>((resolve, reject) => {
        ...
        ollama.stdout.on("data", (data: Buffer) => {
          ...
          console.log(
            `Chunk recibido: ${data.length} chars, total: ${stdout.length}`,
          );
        });
        ...
        ollama.on("close", (exitCode: number) => {
          ...
          if (exitCode !== 0) {
            ...
            reject(
              new Error(`Ollama process failed with exit code ${exitCode}`),
            );
            return;
          }
      ...
      throw new Error(
        `OLLAMA CONNECTION FAILED: ${error instanceof Error ? error.message : error}. ` +
   ...
   */
  private detectFixes(original: string, processed: string): string[] {
    const fixes: string[] = [];
...
`;
   ...
   * Generate final APB log
   */
  private async generateFinalAPBLog(projectPath: string): Promise<void> {
